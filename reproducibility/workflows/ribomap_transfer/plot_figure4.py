...
def _draw_bias(ax, bias: pd.DataFrame, tests: pd.DataFrame | None = None) -> None:
    ...
    display_labels = ["Deep-RIBOmap", "Shared", "STARmap", "Background"]
    ...
    for comparison_index, (left, right) in enumerate(comparisons):
        if len(values[left]) and len(values[right]):
            ...
            qvalue = float(row["qvalue_bh"]) if row is not None and "qvalue_bh" in row else np.nan
    ...
    ax.set_xticks(range(4), display_labels, rotation=30, ha="right")
