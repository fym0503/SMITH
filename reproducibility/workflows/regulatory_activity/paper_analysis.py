...
def normalize_module_table(source: str | Path | pd.DataFrame) -> pd.DataFrame:
    """Normalize atlas module annotations supplied as a path or in-memory table."""
    frame = source.copy() if isinstance(source, pd.DataFrame) else pd.read_csv(source, sep="\t")
    ...
    if module is None or gene is None:
        raise ValueError("Module annotation lacks module and TF columns")
    ...
    if result.empty:
        raise ValueError("Module annotation contains no usable rows")
...
def normalize_tf_pairs(source: str | Path | pd.DataFrame | None, genes: list[str]) -> pd.DataFrame:
    if source is None:
        return pd.DataFrame(
    ...
    if left is None or right is None:
        raise ValueError("TF-pair annotation lacks TF1/TF2 columns")
    ...
    if result.empty:
        raise ValueError("No TF pairs overlap the activity input")
    return result.drop_duplicates()
...
def coactivity_from_objects(
    train: ad.AnnData,
    test: ad.AnnData,
    panel_genes: list[str],
    *,
    pairs: str | Path | pd.DataFrame | None = None,
    ...
    """Compare reconstructed and observed TF-pair profiles from current objects."""
    ...
    requested_panel_size = len(panel_genes)
    panel = [canonical_tf_name(gene) for gene in panel_genes if canonical_tf_name(gene) in common]
    ...
    if reconstructed is not None:
        ...
        backend = reconstruction_backend or "provided_reconstruction"
        ...
        reconstructed_collapsed, reconstructed_genes = _collapse_columns(reconstructed_raw, test_names)
        reconstructed_index = {gene: index for index, gene in enumerate(reconstructed_genes)}
        reconstructed = reconstructed_collapsed[:, [reconstructed_index[gene] for gene in common]]
    ...
    pair_table = normalize_tf_pairs(pairs, common)
    rows = []
    rng = np.random.default_rng(seed)
    for lineage in LINEAGE_NAMES:
        tissue_pairs = pair_table[pair_table["tissue"].isin([lineage, "all"])].copy()
        ...
        if max_pairs and len(tissue_pairs) > max_pairs:
            tissue_pairs = tissue_pairs.iloc[np.sort(rng.choice(len(tissue_pairs), size=max_pairs, replace=False))]
        ...
        rows.append(
            {
                "split": "test", "lineage": lineage, "method": method,
                "panel_size": requested_panel_size, "n_canonical_panel_tfs": len(panel),
                "pair_scope": "annotated" if pairs is not None else "all", "backend": backend,
                "n_cells": int(mask.sum()), "n_pairs": int(len(tissue_pairs)), "pearson": correlation,
            }
        )
    if not rows:
...
def coactivity_reconstruction(
    ...
    *,
    ...
    """File-based CLI wrapper around :func:`coactivity_from_objects`."""
    train, test = ad.read_h5ad(train_file), ad.read_h5ad(test_file)
    reconstruction = None
    if reconstruction_file:
        common = sorted(
            set(map(canonical_tf_name, gene_symbols(train)))
            & set(map(canonical_tf_name, gene_symbols(test)))
        )
        reconstruction = _load_reconstruction(reconstruction_file, common)
    result = coactivity_from_objects(
        train, test, read_panel(panel_file), pairs=pair_file, lineage_column=lineage_column,
        max_pairs=max_pairs, seed=seed, method=method, checkpoint_file=checkpoint_file,
        reconstructed=reconstruction,
        reconstruction_backend="persist_reconstruction_head" if reconstruction_file else None,
    )
    output_file = Path(output_file)
    output_file.parent.mkdir(parents=True, exist_ok=True)
    result.to_csv(output_file, sep="\t", index=False)
...
def tf_scrna_correlation_from_objects(
    scrna: ad.AnnData,
    tf: ad.AnnData,
    *,
    ...
    """Calculate cross-modality TF correlation from loaded AnnData objects."""
    scrna_source_names = (
    ...
    cluster_count = min(n_clusters, len(shared_genes))
    clustering = SpectralBiclustering(
        n_clusters=(cluster_count, cluster_count), method="log", random_state=random_state
    ).fit(tf_correlation)
    order = np.lexsort((np.asarray(shared_genes), clustering.column_labels_, clustering.row_labels_))
    matrices = {
        "genes": np.asarray(shared_genes)[order],
    ...
    summary = pd.DataFrame(
        [{
            "shared_genes": len(shared_genes), "shared_lineages": len(shared_lineages),
            "pearson": mean_rowwise, "mean_rowwise_pearson": mean_rowwise,
...
def tf_scrna_correlation(
    ...
    *,
    ...
    """File-based CLI wrapper around :func:`tf_scrna_correlation_from_objects`."""
    scrna, tf = ad.read_h5ad(scrna_file), ad.read_h5ad(tf_file)
    summary, matrices = tf_scrna_correlation_from_objects(
        scrna, tf, tf_lineage_column=tf_lineage_column,
        scrna_lineage_column=scrna_lineage_column, n_clusters=n_clusters,
        random_state=random_state,
    )
    output_file = Path(output_file)
    output_file.parent.mkdir(parents=True, exist_ok=True)
    matrix_file = output_file.with_name("figure3_j_correlation_matrices.npz")
    np.savez_compressed(matrix_file, **matrices)
    summary = summary.assign(matrix_file=matrix_file.name)
