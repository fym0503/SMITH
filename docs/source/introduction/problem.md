...
A canonical marker may separate known cell classes but miss within-class state. A highly expressed target may be easy to detect but spatially uninformative. The **right subset depends on the intended biological analysis**, not on abundance or generic marker status alone.
