...
from smith_agent.feasibility import MANUSCRIPT_FIGURE6G_GENES
...
'''
    load = '''relative_inputs = [
    "agent/liver_merfish/cellxgene_liver_scrna.h5ad",
    "agent/liver_merfish/adata_healthy_nucseq.h5ad",
    "agent/liver_merfish/adata_healthy_merfish.h5ad",
    "agent/references/PSC011_C1_visium.h5ad",
    "agent/references/WSSS_F_IMMsp9838712_visium.h5ad",
...
source_probe_raw, source_raw, merfish, ref_a_raw, ref_b_raw = [ad.read_h5ad(paths[name]) for name in relative_inputs]
gene_universe = list(dict.fromkeys(str(gene).upper() for gene in merfish.var_names if str(gene).strip()))
source = prepare_agent_adata(source_raw, gene_universe, max_cells=MAX_CELLS, seed=1)
ref_a = prepare_agent_adata(ref_a_raw, gene_universe, require_spatial=True, max_cells=MAX_CELLS, seed=1)
ref_b = prepare_agent_adata(ref_b_raw, gene_universe, require_spatial=True, max_cells=MAX_CELLS, seed=1)
...
# Figure 6f-g uses the complete CellxGene liver candidate universe. This is
# ranked from the freshly loaded H5AD with the same ranking weights used by the
# manuscript workflow; it is separate from the MERFISH shared universe used
# for Figure 6c-d panel evaluation.
source_probe_ranking = rank_genes_from_adata_loaded(
    source_probe_raw,
    dataset_id="cellxgene_liver_scrna",
    data_role="source_scrna_probe_feasibility",
    symbol_column="feature_name",
    max_cells=int(os.environ.get("SMITH_TUTORIAL_RANK_MAX_CELLS", "50000")),
    seed=1,
...
'''
    cells = [
        nbformat.v4.new_markdown_cell(f"# {spec['title']}\n\n{spec['biology']}\n\nThis notebook is a fixed, reproducible SMITH-Agent workflow rather than code generated by an online planner. It trains source and spatial-reference models from the downloaded H5AD files, combines their in-memory rankings, tunes the evidence balance on held-out MERFISH biology, and checks whether genes in the selected panel can support probe design."),
        nbformat.v4.new_markdown_cell("## Download the source data\n\n```bash\npython scripts/download_tutorial_data.py --case 05_agent --data-root data/tutorials\n```\n\nThe five H5AD files are real liver source, target, reference and complete candidate-universe inputs. The notebook creates its ranking and shared-gene objects in memory; it does not load a prior panel, metrics file, or prepared-data artifact."),
...
display(figure)
plt.close(figure)'''),
        nbformat.v4.new_markdown_cell("## Figure 6f: package-level probe feasibility\n\nThe manuscript evaluates the complete ranked liver scRNA candidate universe, rather than only the selected panel. ODT property designability, OligoMiner gene-aware specificity and ProbeDealer target compatibility are run from the current SMITH ranking. The integrated bar is the intersection of the three live gates; missing external backends stop the cell with an actionable error."),
        nbformat.v4.new_code_cell('''probe_analysis = run_probe_feasibility_loaded(
    source_probe_ranking,
    CASE_OUTPUT / "probe_feasibility",
    project_root=ROOT,
    species="homo_sapiens",
    human_reference_dir=os.environ.get("SMITH_HUMAN_REFERENCE_DIR"),
    gene_metadata_h5ad=paths["agent/liver_merfish/cellxgene_liver_scrna.h5ad"],
    max_genes=12160,
    force=True,
)
pass_rates = probe_analysis["pass_rates"]
...
display(figure)
plt.close(figure)'''),
        nbformat.v4.new_markdown_cell("## Figure 6g: representative ProbeDealer off-target profiles\n\nThe same current ProbeDealer run is decomposed for the representative genes shown in the manuscript. Each bar separates target-compatible probes, known different-symbol off-targets and probes whose annotation or transcriptome hit remains unknown."),
        nbformat.v4.new_code_cell('''examples = probe_analysis["examples"]
figure, axis = plt.subplots(figsize=(3.7, 3.55), facecolor="white")
...
figure.text(0.015, 0.985, "g", ha="left", va="top", fontsize=10, weight="bold")
figure.subplots_adjust(left=0.22, right=0.99, top=0.98, bottom=0.27)
display(figure)
plt.close(figure)'''),
    ...
    "agent_plan": agent_plan, "outputs": {"figure6": str(FIGURE_DATA / "figure6_c_d_values.tsv"), "tuning": str(FIGURE_DATA / "agent_parameter_tuning.tsv"), "probe": probe_analysis["audit"]},
    "probe_backend": {"status": probe_analysis["status"], "scope": "manuscript_figure6_f_g", "artifacts": probe_analysis["backend_outputs"]},
})'''),
