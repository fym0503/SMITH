...
display(figure)
plt.close(figure)'''),
        nbformat.v4.new_markdown_cell("## Example: check probe feasibility for selected genes\n\nThe selected panel is now passed to a real transcript-resolution and ProbeDealer sequence-property screen. The plot reports candidate counts for the first 16 selected genes and the live status of the configured backends. This is a feasibility example, not a fabricated pass rate: transcriptome-wide OligoMiner/BLAST specificity and PaintSHOP scoring are shown as unavailable until those external backends are installed."),
